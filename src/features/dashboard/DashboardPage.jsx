...
import { CheckCircle2, XCircle, Clock, PauseCircle, Calculator, Target } from "lucide-react";
...
const extrairDataISORegistro = (registro) => {
  ...
  if (correspondencia) {
    return [correspondencia[1], correspondencia[2], correspondencia[3]].join("-");
  }
...
export default function Dashboard() {
  ...
  const tiposDisponiveis = useMemo(() => {
    return ordenarValores(rawDados.map((registro) => registro.tipo));
  }, [rawDados]);
  ...
  const produtosDisponiveis = useMemo(() => {
    ...
    return ordenarValores(baseProdutos.map((registro) => registro.cod_prod));
  }, [rawDados, filtros.injetora]);

  const dadosFiltrados = useMemo(() => {
    return rawDados.filter((registro) => {
      ...
      if (filtros.dataInicio) {
        if (!dataRegistro || dataRegistro < filtros.dataInicio) {
          return false;
        }
      }

      if (filtros.dataFim) {
        if (!dataRegistro || dataRegistro > filtros.dataFim) {
          return false;
        }
      }
  ...
  }, [rawDados, filtros.injetora, filtros.cod_prod, filtros.dataInicio, filtros.dataFim]);
  ...
  const percentualHoraTrabalhada =
    horasTotaisDec > 0 ? (Number(metrics?.horasTrabalhadasDec || 0) / horasTotaisDec) * 100 : 0;

  const percentualHoraParada =
    horasTotaisDec > 0 ? (Number(metrics?.horasParadasDec || 0) / horasTotaisDec) * 100 : 0;

  const maiorMotivo = metrics?.motivos?.[0]?.value || 0;
        ...
        <section className="kpi-grid">
          ...
          <div className="kpi-card verde kpi-horas-trabalhadas">
            <div className="dias-trabalhados-indicador">
              ...
              <strong>{metrics?.diasTrabalhados || "0.00"}</strong>
            </div>
            ...
            <div className="percentual-horas-indicador percentual-horas-indicador-trabalhadas">
              {formatarPercentual(percentualHoraTrabalhada)}%
            </div>
          ...
          <div className="kpi-card vermelho kpi-horas-trabalhadas">
            <div className="dias-trabalhados-indicador">
              ...
              <strong>{metrics?.diasParados || "0d 00h"}</strong>
            </div>
            ...
            <div className="percentual-horas-indicador percentual-horas-indicador-paradas">
              {formatarPercentual(percentualHoraParada)}%
            </div>

            <strong className="valor-horas-trabalhadas">
              {metrics?.horasParadas || "00:00"} hrs
            </strong>
          </div>
          ...
          <div className="kpi-card verde kpi-horas-trabalhadas">
            <div className="dias-trabalhados-indicador">
              ...
              <strong>{metrics?.diasTotais || "0d 00h"}</strong>
            </div>
